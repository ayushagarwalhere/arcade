...
import type { AgentKind, AgentStatus, FindingStatus, Severity, SurfaceNode } from "@arcade/core/types";
...
export function Panel({ title, right, children, className = "", bodyClass = "" }: { title?: React.ReactNode; right?: React.ReactNode; children: React.ReactNode; className?: string; bodyClass?: string }) {
  return (
    <section className={`overflow-hidden rounded-md border border-ade-line bg-ade-base ${className}`}>
      {title && (
        <header className="flex items-center justify-between gap-3 border-b border-ade-line px-4 py-3">
...
export function Metric({ label, value, unit, tone = "neutral", hint }: { label: string; value: string | number; unit?: string; tone?: Tone; hint?: string }) {
  ...
  return (
    <div className="rounded-md border border-ade-line bg-ade-base px-4 py-3.5">
