...
const PLATFORMS = [
  { os: "Windows", detail: "x64 · portable", Icon: Monitor, href: SITE.downloads.windows, ready: true },
  { os: "macOS", detail: "Apple Silicon", Icon: AppleIcon, href: SITE.releases, ready: false },
  { os: "macOS", detail: "Intel", Icon: AppleIcon, href: SITE.releases, ready: false },
  { os: "Linux", detail: "AppImage", Icon: TerminalSquare, href: SITE.releases, ready: false },
  { os: "Linux", detail: ".deb", Icon: TerminalSquare, href: SITE.releases, ready: false },
];
      ...
      <Reveal delay={0.05} className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {PLATFORMS.map((p) => (
          ...
          >
            ...
            <span className={`text-[13px] font-medium ${p.ready ? "text-emerald-300" : "text-white/45"} group-hover:text-white/90`}>
              {p.ready ? "Download" : "Releases"}
            </span>
          </a>
        ))}
        <a
