...
export default function DownloadActions({ secondary = "github" }: { secondary?: "github" | "discord" }) {
  ...
  return (
    <div id="download" className="flex flex-col items-center scroll-mt-32">
      <div className="flex flex-wrap items-center justify-center gap-3">
        ...
        {secondary === "github" ? (
          <a href={SITE.github} className="flex items-center gap-3 rounded-xl border border-white/10 bg-black px-7 py-4 text-[17px] font-medium text-white transition hover:bg-white/[0.06]">
            <GithubIcon className="h-5 w-5" /> View on GitHub
          </a>
        ) : (
          <a href={SITE.discord} className="flex items-center gap-3 rounded-xl border border-white/10 bg-black px-7 py-4 text-[17px] font-medium text-white transition hover:bg-white/[0.06]">
            <DiscordIcon className="h-5 w-5" /> Join the Discord
          </a>
        )}
      </div>
      <div className="relative mt-5">
        <button onClick={() => setOpen((o) => !o)} className="flex items-center gap-1.5 text-[14px] text-white/50 transition hover:text-white/80">
        ...
        {open && (
          <div className="absolute left-1/2 top-8 z-20 w-56 -translate-x-1/2 rounded-xl border border-white/10 bg-ink-800 p-1.5 text-left shadow-2xl animate-fade-in">
            {PLATS.map((p) => (
              <a key={p.id} href={SITE.downloads[p.id]} className="block rounded-lg px-3 py-2 text-sm text-white/80 hover:bg-white/[0.07]">
