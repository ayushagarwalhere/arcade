...
import type { ArcadeState, SurfaceNode } from "@/lib/arcade/types";
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[1fr_300px]">
      <div>
        ...
        <div className="scrollbar-thin overflow-x-auto rounded-xl border border-white/[0.08] bg-ink-900 bg-grid">
                ...
                return (
                  <g key={i}>
                    <path
                      ...
                      fill="none"
                      stroke={vuln ? "#ef4444" : "rgba(255,255,255,0.16)"}
            ...
            {surface.nodes.map((n, i) => {
              ...
              return (
                <button
                  ...
                  className={`absolute flex h-[52px] w-[156px] animate-fade-in items-center gap-2.5 rounded-xl border bg-ink-850 px-3 text-left transition ${RISK_RING[n.risk]} ${
      ...
      <div>
        {sel ? (
          <div className="rounded-xl border border-white/[0.08] bg-ink-900 p-4">
            ...
            {sel.id === "admin" && (
              <div className="mt-3 rounded-lg border border-red-500/25 bg-red-500/[0.06] px-3 py-2 text-[11.5px] text-red-200/90">
                Finding ARC-001 lives here. The proven exploit path runs User → Browser → API → Admin Export → PostgreSQL.
              </div>
            )}
          </div>
        ) : (
          <div className="rounded-xl border border-white/[0.08] bg-ink-900 p-4 text-[13px] text-white/45">
