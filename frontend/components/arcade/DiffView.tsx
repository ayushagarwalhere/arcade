...
import type { RemediationFile } from "@/lib/arcade/types";
...
export default function DiffView({ files, commit, summary }: { files: RemediationFile[]; commit?: string; summary?: string }) {
  if (!files.length) {
    return <div className="rounded-xl border border-dashed border-white/12 bg-ink-900 p-8 text-center text-[13px] text-white/45">No changes yet. A diff appears after the fix is approved.</div>;
  }
