...
import type { TimelineEvent, TimelineKind } from "@/lib/arcade/types";
...
export default function TimelineList({ events, empty = "Nothing yet." }: { events: TimelineEvent[]; empty?: string }) {
  if (!events.length) {
    return <div className="rounded-xl border border-dashed border-white/12 bg-ink-900 p-6 text-center text-[13px] text-white/45">{empty}</div>;
  }
  return (
      ...
      {events.map((e, i) => {
        ...
        return (
          <li key={i} className="relative flex animate-fade-in gap-3.5 pb-4">
            {!last && <span className="absolute left-[15px] top-8 h-[calc(100%-16px)] w-px bg-white/[0.09]" />}
            <span className={`relative z-10 grid h-8 w-8 shrink-0 place-items-center rounded-lg ${k.color}`}>
